...
namespace MyTemplatorTests
{
    ...
    public class SimpleTemplateEngineTests
    {
        ...
        // Тест на отсутствие выполнения условия (if), если условие ложное
        [Test]
        public void Render_ShouldNotProcessIfConditionWhenFalse()
        {
            var template = "Condition: {{#if IsValid}}Valid{{/if}}";
            var data = new { IsValid = false };

            var result = _engine.Render(template, data);

            Assert.IsFalse(result.Contains("Valid"));
        }

        // Тест на обработку условий (if) с пустым значением
        [Test]
        public void Render_ShouldNotProcessIfConditionWhenNull()
        {
            var template = "Condition: {{#if IsValid}}Valid{{/if}}";
            var data = new { IsValid = (bool?)null };
